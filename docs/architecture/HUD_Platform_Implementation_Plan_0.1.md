...
| `assets.nebulacast.app` | Does **not** exist yet. Referenced only in the arch docs. Stories #6/#7 create it. |
...
| 6 | CDN / Registry deployment | A dedicated **Cloudflare Pages** project for `assets.nebulacast.app` (custom domain on the `nebulacast` zone), deployed by a new `scripts/deploy/` tool **modelled on `deploy_gadgets_media.sh`** (direct `wrangler pages deploy` of a validated `dist/`, no GitHub connection). Reuse the pattern; do not invent a new mechanism. | Memory: reuse existing infra, don't stand up parallel CDN targets. Arch §33 allows "the most appropriate Wrangler-compatible mechanism"; Pages direct-upload is the one already proven here. **Owner decision needed** — see §7. |
...
| Cloudflare account / `nebulacast` DNS zone | A Pages project + `assets.nebulacast.app` custom domain + a `wrangler` token. **Owner / DevOps action.** | #6, #7 |
| One external consumer — candidate: **`nebulacast-site`** (Blogger-style pages are the stated motivating case) or **`stellar-attractor-site`** (its Phase 2 HUD work is parked and would consume this) | An implementation-agnostic integration using only the public Runtime/Contract API. | #17 |
