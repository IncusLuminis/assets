...
const themeDir = path.resolve(__dirname, "..", "..", "library", "themes", "hud-01");

function loadManifest() {
  return JSON.parse(fs.readFileSync(path.join(themeDir, "manifest.json"), "utf8"));
}

describe("library/themes/hud-01 -- 'Object Lock' (Story #11 AC)", () => {
  ...
  it("id is the reserved hud-01 baseline id, engine svg, isolation scoped-root (Contract §27.8 rule 9)", () => {
    const manifest = loadManifest();
    expect(manifest.id).toBe("hud-01");
    expect(manifest.engine).toBe("svg");
    expect(manifest.isolation).toBe("scoped-root");
  });

  it("declares overflowVisible: true (4-layer contour-glow runner paints outside its box, Contract §15.5)", () => {
    const manifest = loadManifest();
  ...
  it("declares skyViewer + all three dataSource providers (Contract §10, matching §25.1's HUD-01 cross-check row)", () => {
    const manifest = loadManifest();
