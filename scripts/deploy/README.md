...
3. stages an **allowlist-only** copy — `dist/themes/`, `dist/runtime/`,
   the repo-root `registry/index.json`, and `scripts/deploy/_headers` — into
   gitignored `.deploy/` (nothing else from `dist/` or the repo can reach
   the CDN, Arch §44);
...
## Cache policy (Arch §34, issue #6 AC)

`scripts/deploy/_headers` is a committed Cloudflare Pages [`_headers`
file](https://developers.cloudflare.com/pages/configuration/headers/) —
staged to `.deploy/_headers`, Cloudflare reads it natively at publish time,
no separate CDN-side config needed:

- `/themes/*` — `max-age=31536000, immutable`. Every Theme asset lives
  under an immutable version directory (`/themes/<id>/<version>/...`,
  Contract 1.0), so it's safe to cache forever.
- `/registry/index.json` — `max-age=60, must-revalidate`. The one mutable,
  frequently-re-read entry point new Theme versions get published through;
  short TTL so consumers see a newly published Theme quickly.
- `/runtime/*` — `max-age=300, must-revalidate`. Published at a fixed,
  non-versioned path in 0.1 (see below), so unlike Theme assets it is
  *not* immutable at that path and gets a short TTL too.

## Why `registry/index.json` and `_headers` are staged from outside `dist/`
...
`media/` yet). `scripts/deploy/_headers` (cache policy, above) is likewise
a committed source file, not build output. So the staging step in
`deploy-registry.sh` explicitly copies from four separate allowlisted
sources — `dist/themes/`, `dist/runtime/`, repo-root `registry/index.json`,
and `scripts/deploy/_headers` — rather than mirroring `dist/` as a single
blanket copy. The resulting published tree (`themes/`, `runtime/`,
`registry/index.json`, `_headers` all at the site root) matches
