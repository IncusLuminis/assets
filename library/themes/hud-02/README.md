...
(`.nc-or-panel`), and a real YouTube/HeyGen iframe there is either
...
- `mediaPoster` is consumed directly out of `setData()`'s data argument,
  ...
  any composition's markup, by design.
...
See `tests/unit/hud-02-e2e.test.js`'s "Story #45" describe block for the
