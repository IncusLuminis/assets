// hud-02/scripts/hud-02.js -- Theme entry script for maxi:landscape.
...
// widgets/sandbox/blogger-hud01-02-wip/blogger-hud02-template.js's
// `createInstance()` factory -- structurally identical to hud-01's own
// scripts/hud-01.js (Story #11) since the two real baselines share almost
// all of this logic (same SIMBAD/VizieR/ADS query shapes, same Aladin
// options, same shared `NcHudMini` toggle); the only real hud-02-specific
// deltas are the `nc-or-*` class/id namespace and the DATA panel's
// 4-column grid (a markup/CSS concern, not a script one). The blogger
// lineage's multi-instance bookkeeping (`_ncAladinLoader` page global,
// unique-id counters) is replaced here by SvgRenderer's own per-instance
// `#runThemeScript` call (fresh closure per mount) and
// `DomExternalResourceLoader`'s page-scoped `kind:host` de-duplication
// (Story #9) -- so this script itself carries no cross-instance state;
// `capabilities.multiInstance: true` still holds because every DOM query
// below is scoped to `root`, matching Contract §16.5's requirement for a
// Theme claiming multi-instance support.
"use strict";
...
return (function () {
  ...
  // verbatim from the real baseline's own designed safety net (identical
  // fallback rows to hud-01's; both Themes' real scripts hardcode the same
  // NGC 1300 demo data).
  ...
  function warn(msg) { if (typeof console !== "undefined") console.warn("[hud-02] " + msg); }
  ...
  function injectDataRows(pairs, isFallback) {
    ...
    pairs.forEach(function (pair) {
      var lbl = document.createElement("div"); lbl.className = "nc-or-label"; lbl.textContent = pair[0] + ":";
      var val = document.createElement("div"); val.className = "nc-or-value"; val.textContent = pair[1];
  ...
  function setDataStatus(text, state) {
    ...
    if (state === "loading") { var ring = document.createElement("div"); ring.className = "nc-or-spinner"; wrap.appendChild(ring); }
  ...
  function loadVizierReferences(target) {
    ...
    vizierBusy = true;
    setPanelStatus(".nc-or-info-panel--catalog", "LOADING VIZIER REFERENCES…", "loading");
  ...
  function renderVizierRefs(refs, isFallback) {
    ...
    refs.forEach(function (ref) {
      var card = document.createElement("div"); card.className = "nc-or-paper-card";
      ...
      if (ref.desc) { var desc = document.createElement("div"); desc.className = "nc-or-paper-abstract"; desc.textContent = ref.desc; card.appendChild(desc); }
  ...
  function renderPapers(papers) {
    ...
    panel.appendChild(hdr);
    if (!papers.length) { setPanelStatus(".nc-or-info-panel--papers", "NO PUBLICATIONS FOUND", "error"); return; }
  ...
  //    option in this 0.1 Runtime (same gap/workaround as hud-01's). ─────
  ...
  return {
    ...
    // of this (Inv §6.5, H10) -- 1.0 requires it, so this is new
